...
void totaladd(int thous, int fives, int twos, int hunds, int fiftys)
    {
        struct atm a;
        FILE *pntr;
        pntr = fopen("totalamount\\totalamount.dat", "rb");
        if (pntr == NULL)
            {
        ...
        while(fread(&a, sizeof(a), 1, pntr))
            {
                FILE *ptr5;
                ptr5 = fopen("totalamount\\details.dat","ab");
        ...
        fclose(pntr);
        remove("totalamount\\totalamount.dat");
        rename("totalamount\\details.dat", "totalamount\\totalamount.dat");
...
void totalsub(int thous, int fives, int twos, int hunds, int fiftys)
    {
        struct atm a;
        FILE *ptnr1;
        ptnr1 = fopen("totalamount\\totalamount.dat", "rb");
        if (ptnr1 == NULL)
            {
                printf("Error Occured while opening the FILE...!");
            }
        while(fread(&a, sizeof(a), 1, ptnr1))
            {
                FILE *ptr7;
                ptr7 = fopen("totalamount\\details.dat","ab");
                a.thous = a.thous - thous;
                a.fives = a.fives - fives;
                a.twos = a.twos - twos;
                a.hunds = a.hunds - hunds;
...
void display()
    {
        struct atm a;
        FILE *ptr6;
        ptr6 = fopen("totalamount\\totalamount.dat", "rb");
        if (ptr6 == NULL)
            {
